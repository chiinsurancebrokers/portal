...
def _brevo_smtp_send(to_email: str, to_name: str, subject: str, body_html: str) -> tuple:
    ...
    if brevo_user and brevo_pass:
        try:
            msg = _build_msg("info@chiinsurancebrokers.com")
            with smtplib.SMTP("smtp-relay.brevo.com", 587, timeout=20) as srv:
                srv.ehlo(); srv.starttls(); srv.ehlo()
                srv.login(brevo_user, brevo_pass)
                srv.sendmail("info@chiinsurancebrokers.com", [to_email], msg.as_string())
            return True, ""
        except Exception as e:
            pass  # fall through to Gmail

    # 2. Fallback: Gmail SMTP (App Password)
    ...
    return False, "Δεν έχουν οριστεί SMTP credentials (BREVO_SMTP_USER/PASS ή GMAIL_USER/GMAIL_APP_PASS)"
