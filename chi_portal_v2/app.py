...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            cid                = user.client_id
            agent_scope        = user.agent_code
            must_change        = bool(user.must_change_password) if user.must_change_password is not None else True
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not existing_admin:
                ...
                existing_admin.name = "Chris Iatropoulos"
...
def _parse_lixiario_csv(file_bytes):
    ...
    for line in data_lines:
        ...
        # Map numeric agent codes to names
        raw_agent = parts[12].strip()
    ...
    try:
        ...
        # ── Policy (new or renewal) ───────────────────────────────────────────
            ...
            if prem > 0 and expiry:
                db.add(m.Payment(policy_id=policy.id, amount=prem, due_date=expiry, status=m.PaymentStatus.PENDING))
            db.commit()
...
def admin_agents():
    ...
    db = m.get_session()
    try:
        agents = db.query(m.Agent).filter(m.Agent.is_admin == False).order_by(m.Agent.code).all()
        data = []
        for ag in agents:
...
def admin_create_agent_user(agent_id):
    ...
    ag = db.query(m.Agent).get(agent_id)
    if not ag: db.close(); abort(404)
    email = request.form.get("email") or ag.email
    pw    = request.form.get("password","chi2026!")
    try:
        existing = db.query(m.User).filter_by(email=email).first()
        if existing:
            existing.password_hash = generate_password_hash(pw)
            existing.agent_code    = ag.code
            ...
            db.add(m.User(email=email, password_hash=generate_password_hash(pw),
                          role=m.UserRole.AGENT, name=ag.name, agent_code=ag.code))
...
<tr><td style='background:#F8FAFC;padding:16px 32px;border-top:1px solid #E2E8F0'>
  <div style='font-size:11px;color:#94A3B8'>CHI Insurance Brokers · info@chiinsurancebrokers.com</div>
</td></tr>
</table></body></html>"""
    try:
        resp = req.post("https://api.brevo.com/v3/smtp/email",
            ...
            json={"sender": {"name":"CHI Insurance","email":"info@chiinsurancebrokers.com"},
                  ...
                  "subject": "Επαναφορά Κωδικού — CHI Insurance Portal",
                  "htmlContent": body_html}, timeout=15)
