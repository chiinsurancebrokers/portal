...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            db.commit()
            db.close()
            agent_scope = user.agent_code  # None = admin, 'ca'/'3p'/etc = restricted
            session["user_id"]    = uid
            session["role"]       = role
...
def admin_agents():
    ...
    if get_agent_scope() is not None:
        ...
        db.close(); return redirect(url_for("agent_dashboard"))
    agents = db.query(m.Agent).order_by(m.Agent.code).all()
    data = []
    for ag in agents:
        user = db.query(m.User).filter_by(agent_code=ag.code).first()
        pol_count = db.query(m.Policy).filter(
        ...
        data.append({
            ...
            "pol_count": pol_count, "clients_count": clients_count,
            "total_premium": total_premium, "commission": commission,
        })
    db.close()
...
def admin_add_agent():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.close()
    return render_template("admin/agent_form.html", agent=None, action="add")
...
def admin_edit_agent(agent_id):
    ...
    return render_template("admin/agent_form.html", agent=ag_d,
                           user_email=user.email if user else None, action="edit")
