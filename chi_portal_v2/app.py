...
def _enforce_installments(db, policy, *, force_rebuild=False):
    ...
    inst_amt  = round(annual / max_n, 2) if max_n and annual else 0
    ...
    paid = db.query(m.Payment).filter(
        m.Payment.policy_id == policy.id,
    ...
    if total < max_n and annual > 0:
        ...
        if n_paid == 0:
            ...
            for i in range(1, missing + 1):
                ...
                created += 1
            return ("added_missing", 0, created)
...
def agent_edit_policy(policy_id):
    ...
    if request.method == "POST":
        try:
            ...
            # Enforce installment rules on save — NEVER force rebuild (preserves PAID)
            action, n_del, n_new = _enforce_installments(db, policy)
            db.commit()
            ...
            inst_amt = round(prem / max_n, 2) if max_n and prem else 0
            if action == "trimmed":
                flash(f"✅ Συμβόλαιο ενημερώθηκε. Αφαιρέθηκαν {n_del} περιττές δόσεις → {max_n} {freq_label}.", "success")
            elif action == "rebuilt":
                flash(f"✅ Συμβόλαιο ενημερώθηκε. {max_n} δόσεις {freq_label} (€{inst_amt:.2f}).", "success")
            elif action == "added_missing":
                flash(f"✅ Συμβόλαιο ενημερώθηκε. Προστέθηκαν {n_new} δόσεις.", "success")
            else:
...
def agent_payments():
    ...
    try:
        ...
        # Filter by Payment.due_date month/year at SQL level (not policy expiry)
        if month_f:
            try:
                from sqlalchemy import extract as _ext
                q = q.filter(_ext("month", m.Payment.due_date) == int(month_f))
            except Exception:
                pass
        if year_f:
            try:
                from sqlalchemy import extract as _ext
                q = q.filter(_ext("year",  m.Payment.due_date) == int(year_f))
            except Exception:
                pass

        payments = q.order_by(m.Payment.due_date.asc()).all()
        ...
        for pay in payments:
            ...
            data.append({
                ...
                "pay_month_lbl": (["","Ιαν","Φεβ","Μαρ","Απρ","Μαι","Ιουν","Ιουλ","Αυγ","Σεπ","Οκτ","Νοε","Δεκ"][pay.due_date.month] + " " + str(pay.due_date.year)) if pay.due_date else "—",
