...
def setup():
    ...
    if request.method == "POST":
        ...
        if not db.query(m.User).filter_by(email="info@chiinsurancebrokers.com").first():
            ...
            db.commit()
        db.close()
        return "<h2>✅ Setup complete! <a href='/login'>Login</a></h2>"
    return """<form method=POST style='font-family:sans-serif;max-width:400px;margin:60px auto'>
    <h2>CHI Portal v2 — Setup</h2>
    <p>Setup Key: <input name=key type=password style='padding:8px;width:100%'></p>
    <button type=submit style='padding:10px 24px;background:#1B2B5E;color:white;border:none;border-radius:6px;cursor:pointer'>Initialize Database</button>
    </form>"""
    ...
    try:
        ...
        for c in clients:
            ...
            data.append({
                ...
                "sectors": sectors, "next_expiry": next_exp, "pending_payments": pending
            })
        return render_template("agent/clients.html", clients=data, search=search,
                               sector=sector, sectors=[s.value for s in m.PolicySector])
    finally:
        db.close()
...
def agent_renewals():
    ...
    try:
        policies = db.query(m.Policy).filter(
            m.Policy.expiration_date.between(today, cutoff),
            m.Policy.status == m.PolicyStatus.ACTIVE
